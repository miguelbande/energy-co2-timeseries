...
# CARGA Y PREPARACIÓN DE DATOS DE ELECTRICIDAD FRANCESA (MENSUAL)              #
...
plot(load_fr_ts, main="Consumo Eléctrico Francia - MWh", col="blue")
...
load_fr_sdiff <- diff(load_fr_ts, differences=1, lag=12)
load_fr_sdiff_diff <- diff(load_fr_sdiff, differences=1, lag=1)

# Visualizamos la serie diferenciada
par(mfrow=c(1,1))
plot(load_fr_sdiff_diff, 
     main = paste("Serie Estacionaria Final (d=", 1, ", D=", 1, ")", sep=""),
     ylab = "Variación de Demanda (MWh)")
grid()
abline(h=0, col="red", lty=2) 

test_adf <- adf.test(load_fr_sdiff_diff)
print(test_adf)
...
datos_coint <- ts.intersect(
  ...
  Elec = load_fr_sdiff_diff    # Electricidad transformada (estacionaria)
)
